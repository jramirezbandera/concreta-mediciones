...
- [ ] **(diferido) Vista Importar .bc3**: zona drop + selección + tarjetas de bases compatibles; integrar librería `bc3`. Slice posterior (F5.x).
- **Aceptación:** copiar una partida con descomposición crea la partida con `med:[]`, items por código, chip BASE, y sus recursos nuevos entran al banco sin sobreescribir los homónimos ✓; drag&drop al sidebar funciona ✓; en <1100 abre overlay ✓. **Panel Referencia COMPLETO (F5.1 `a257d1c` + F5.2 pendiente); Importar .bc3 diferido.**
