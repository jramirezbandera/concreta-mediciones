...
- [ ] **F6.2 Datos de obra (commit pendiente):** primitiva `Modal` con focus-trap (**cierra T-7**) · `ObraModal` (Obra/Promotor/Constructora/Dirección facultativa; grid 2col / 1móvil; mapa de campos del prototipo `app.jsx:147-150`) · acción `setObraPath('promotor.nif', v)` (paths anidados) · breadcrumb lee `obra.denominacion`.
- [ ] **F6.3 Export/import JSON (commit pendiente):** export `toSerializable→blob→.json` · import `file→parse→validar estructural→fromSerializable→loadObra` con **confirmación + backup previo** (destructivo: reemplaza el único proyecto) y errores legibles (malformado / versión desconocida); si `loadObra` ok pero la escritura IDB falla → avisar.
