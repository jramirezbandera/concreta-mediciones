...
- [x] **F7.1 PDF (HECHO):** `core/listado.ts` (`buildPresupuestoListado` con mediciones embebidas, `buildResumen` —GG/BI redondeados POR LÍNEA para que la hoja sume exacta—, `buildCertListado` con snapshot de F7.0 + contradictorios, y `obraMeta` = contrato de metadatos compartido §4) + selector memoizado `selectResumen`; **vista Resumen EDITABLE** (port de `resumen.jsx`: desglose por capítulos + PEM, GG/BI inline-editables vía `setRates` —único hogar de edición de GG/BI; % a 1 decimal → fracción a 3 decimales (no se pierde el 13,5%, corrección consciente del prototipo)— + IvaSelect + observaciones → **nuevo campo de dominio `obra.notes`** persistido; el tab deja de ser placeholder; estado vacío con CTA, no hoja a 0,00); **`ResumenSheet` compartida** vista↔doc (el doc la renderiza `readOnly` desde el mismo selector); `features/print/` (PrintDoc en portal a `body` con `data-theme="light"` —papel SIEMPRE claro— + PrintPresupuesto/PrintCert; `@page` A4, `<thead>` repetido, `break-inside: avoid`, SIN inputs) montado bajo demanda: espera fuentes + doble rAF → `window.print()` → desmonta en `afterprint` con fallback por timeout y guarda anti-doble-print (StrictMode); `body.printing` oculta `#root` solo mientras el doc existe; **`ExportModal` sobre la primitiva `Modal` (F6.2)** con "mostrar solo lo que funciona" (solo chip PDF para los 3 docs; fila por cert con fecha de snapshot F7.0) y pie explicando que DOCX/XLSX/BC3 llegan por slice. `PlaceholderView` eliminado (las 4 vistas son reales). **QA en navegador real (Chromium headless, `/browse`):** Resumen y modal fieles; PDF del presupuesto paginado con cabecera repetida, capítulos vacíos filtrados y **PEM 26.291,91 € al céntimo**; PDF de cert nº3 llega al líquido (7.145,63 €); ciclo print→afterprint→desmontar verificado. 296 tests verdes. **→ "PDF al instante".**
- [x] **F7.2 XLSX (HECHO):** librería = **`write-excel-file`** (evaluada vs `exceljs`: 1,8 MB vs 21,8 MB unpacked, browser-first, MIT, dep única `fflate`; celdas numéricas + formato + estilos básicos = justo lo necesario), cargada por **import dinámico** → chunk aparte de ~20 kB gzip, fuera del bundle inicial. `features/exportar/xlsxBuilders.ts` (PURO, solo type-imports) construye los 3 docs desde `core/listado` (mismas filas que el PDF: presupuesto combinado con mediciones embebidas, resumen con redondeo por línea, cert con snapshot F7.0 + P.C. + resumen económico hasta el líquido) + `xlsx.ts` (estado→doc + descarga). **Política numérica cumplida y testeada:** cantidades/precios/importes como CELDAS NUMÉRICAS `type: Number` con formato `#,##0.00` (Excel lo pinta es-ES), porcentajes 0–100 con `#,##0.0"%"`, CERO strings preformateados; céntimos→euros con `toEur` en la emisión de la celda (división exacta). Chips Excel (color `--state-ok`, DESIGN.md) junto a PDF en el `ExportModal`. **Verificación con ficheros reales** (builders + `write-excel-file/node` sobre el seed, leídos con openpyxl): 0 strings preformateados, formatos correctos, PEM 26.291,91 / licitación 34.416,11 / líquido cert 3 = 7.145,63 al céntimo; smoke en Chromium real sin errores de consola. 315 tests verdes.
- [x] **F7.3 DOCX (HECHO):** `docx` (v9, MIT, isomórfica) alcanzada SOLO vía import dinámico (`exportDocx` → `docxRender`) → chunk aparte de ~104 kB gzip, fuera del bundle inicial. `docxRender.ts` construye los 3 docs desde `core/listado` con la cabecera de metadatos compartida (§4, mismas rutas que PDF/XLSX) + pie de firma. **Fidelidad Word (el trabajo duro):** página A4 con los márgenes del doc de impresión (14/12/16/12 mm en twips), banda de capítulo + cabecera de columnas marcadas `tableHeader` (se REPITEN en cada página), `cantSplit` en todas las filas (no parten entre páginas), descripciones largas envueltas en celda, mediciones embebidas como líneas grises ("comentario — dims = parcial"), resumen económico de cert alineado a la derecha hasta el líquido. En Word los números son TEXTO → se formatean es-ES con `fmtNum`/`fmtCents` (al revés que XLSX). Chips Word (color `--accent`) en el ExportModal; `fileName.ts` compartido con XLSX. **Tests sobre el .docx REAL** (Packer → unzip con fflate → `word/document.xml`): cabecera/partidas/mediciones presentes, `tblHeader`/`cantSplit` en el XML, A4 + márgenes, PEM/licitación/líquido es-ES al céntimo, precio CONGELADO (F7.0) y no el vivo, P.C. incluidos. **Verificación externa:** los 3 .docx del seed abiertos con `python-docx` (parser OOXML estricto): tablas y filas-cabecera repetibles correctas, totales al céntimo; smoke en Chromium real sin errores. 326 tests verdes.
- [x] **F7.4 BC3 export FIEBDC-3 (HECHO; cierra F7) — PLAN CERRADO (eng-review 2026-06-11, 5 issues D1–D5 + voz externa Codex: 15 hallazgos → 12 refinamientos absorbidos D7 y 2 tensiones resueltas D8/D9; 0 sin resolver). Troceado en BALA TRAZADORA (D9, Codex):** **F7.4a** = spec + writer mínimo (raíz+1 cap+2 partidas con ~D/~M/~T) → **abrirlo en Presto REAL** → GO/ajustar · **F7.4b** = dedupe + anidado + huérfanos + UI + gates por capas.
  - **Writer propio** `src/core/bc3export.ts` (puro: `obraToBc3(ObraData) → Uint8Array`, desde el MODELO DE DOMINIO, no `core/listado`); wrapper `features/exportar/bc3.ts` + botón cabecera «BC3 · obra completa» (`--state-mq`, prototipo) reutilizando `downloadBlob`/`docFileName` (amplía su tipo a `'bc3'`).
  - **SPEC NÚCLEO (D1, anclada al fixture real Presto 8.7 + parser `bc3`):** bytes **windows-1252** declarando `ANSI` en `~V` (encoder propio — `TextEncoder` solo emite UTF-8; no mapeable → `?`); **punto decimal**, sin miles; precios/cantidades 2 dec, rendimientos 3 dec; **CRLF**; orden `~V → ~K → conceptos raíz→hojas (~C/~T/~D/~M)`; raíz `##`, capítulos/subcaps `#`; versión declarada `FIEBDC-3/2016` usando solo registros núcleo; **sanitización** (BC3 no tiene escape): `|`→`¦`, `~` inicial de línea neutralizado, `\`→`/` en campos con subcampos, saltos de línea solo dentro de `~T`. **`~K`:** bloque de decimales de **7 grupos** `\2\2\3\2\2\2\2\EUR\` (corrección Codex — igual que el fixture), pct = `round((coefK−1)·100, 4)`, omitido si K=1; precios de partida/recurso en **BASE** (sin K) y **precio de la raíz = PEM CON K** (espejo exacto del import).
  - **Refinamientos Codex (D7, absorbidos):** `~M` anclado a la DESCOMPOSICIÓN del contenedor (espejo de `collectPartidas`, o el reimport pierde las líneas); `%CI` = concepto `%` con cantidad=porcentaje en `~D` (NO recurso con precio); **tipos `~C` explícitos** (0 estructural · 1 MO · 2 MQ · 3 MAT); precios almacenados de capítulo/sub = Σ hijos (coherentes con la raíz entre lectores); dims de medición hasta 3 dec SIN re-redondear (coherencia ~M↔cantidad por construcción); **dedupe en 2ª pasada TRAS sanitizar/encodear** (sanitizar puede crear colisiones nuevas); `precioManual` documentado (el precio del `~C` es autoridad, la `~D` es informativa — mismo contrato que nuestro import); **recursos huérfanos del banco NO se exportan** (el export es el ÁRBOL de la obra, documentado).
  - **Códigos (D2):** la identidad BC3 ES el código → **renombrado determinista**: homónimos DIVERGENTES (precio/título/desc/items) → sufijo `.2`/`.3`; idénticos → un solo concepto compartido (correcto en BC3); `'——'`/vacíos → códigos generados (`P001`…). **Nada se pierde nunca** (sin esto, Presto fusiona homónimos en silencio).
  - **Subcapítulos (D3): ANIDADOS reales** (el consumidor es Presto, no nuestra app). Asimetría conocida: nuestro import los aplana al reimportar (PEM invariante; documentada y testeada).
  - **DRY (D4):** el exporter usa `core/medicion.lineParcial`; drive-by: `bc3import` deja su copia local (el mapeo 0→'' del import SE QUEDA — es política de mapeo, no aritmética; verificado `medicion.ts:15` + regresión de dims a 0).
  - **Gate de fidelidad POR CAPAS (D5+D8):** (1) units de la spec registro a registro (sanitización, encoder cp1252, dedupe, ~K); (2) round-trip del seed export→`bc3ToObra` **EXACTO al céntimo** (PEM/cantidades/precios + counts — round-trips propios sin tolerancia); (3) **fixture mini ANONIMIZADO commiteado** que ejercita ~V/~K/~C/~D/~M/~T con los quirks de Presto; (4) round-trip de los .bc3 reales **LOCAL-ONLY skip-if-missing** (no están en git: datos de cliente); (5) **gate manual:** abrir el export en Presto real — estructura, acentos, PEM. La tolerancia `<1 €` SOLO aplica al comparar contra el precio raíz escrito por Presto (regla ya testeada del import).
  - **F7.4a HECHO (2026-06-11):** writer `src/core/bc3export.ts` (puro: `obraToBc3(Bc3ExportObra) → Uint8Array`; encoder cp1252 propio con extras 0x80–0x9F, sanitización `|¦`/`\→/`/`~` neutralizado en ~T, ~V/~K/~C/~D/~M/~T según D1+D7; raíz = PEM CON K y capítulo = Σ hijos CON K — verificado contra el fixture: en el propio Presto Σ capítulos = precio raíz al céntimo con las partidas en base; UN ~M por partida en orden para el anclaje por índice, con uds=1 explícito cuando las 4 dims van vacías para que el import no la descarte como línea de sección). **29 tests:** spec registro a registro (encoder, ~K 7 grupos ±pct, sanitización, tipos ~C, %CI=porcentaje, huérfanos fuera) + **round-trip del seed EXACTO al céntimo** (PEM 26.291,91, delta raíz 0, precios/cantidades/rendimientos/mediciones idénticos, con y sin K) + regresión dims-0 (D4). **Bala trazadora `docs/trazadora-presto.bc3`** commiteada (sintética, sin datos de cliente): el test la regenera si el writer cambia (el diff de git delata cualquier cambio de formato). Nota de entorno: el `TextDecoder('windows-1252')` de Node small-icu decodifica 0x80–0x9F como C1 → esos chars se validan a nivel de byte (en navegador real es cp1252 correcto).
  - **Trazadora iteración 1 → AJUSTAR (Presto 8.7 real, 2026-06-11).** El 1er pase validó estructura/acentos/€/sanitización/~M-ancla/K, y cazó DOS errores de spec que el round-trip propio no podía ver: **(1) convención %:** Presto calcula `importe = base × rendimiento` → el rendimiento de un concepto `%` viaja como FRACCIÓN (3 % → `0.03`) con el porcentaje en el campo PRECIO del `~C` (como el fixture: `%PM0200` precio 2, rendimiento 0.02); escribir el porcentaje entero multiplicaba el CI ×100 (precio 12,34 → 444,80). Corregido en export **y drive-by en `bc3import`** (leía 0.02 como cantidad en vez de 2 % — cosmético, el precio del ~C era autoridad en NUESTRO import, pero la justificación mostraba el % mal). **(2) El precio del `~C` NO es autoridad para Presto:** recalcula el padre desde la `~D` (raíz 15.284,81 en vez de 604,93). Regla nueva: la descomposición SOLO viaja si `precioCuadraDescompuesto`; un precio manual/override se exporta como **precio cerrado sin `~D`** (partida alzada — Presto lo respeta, demostrado por TRA020: 8,90 → 186,11). El PEM es lo sagrado; una justificación que no cuadra es irrepresentable en FIEBDC. Presto además redondea `precio×K` por partida (T-8) → su raíz puede diferir céntimos de la nuestra (<1 €, criterio dual D8). Trazadora regenerada (PEM 4.074,13 €; DEM010 cuadra con su descompuesto y ejercita la ~D, TRA020 manual sin ~D). 357 tests verdes.
  - **Trazadora iteración 2 → GO (2026-06-11): validada en Presto 8.7 REAL y, de regalo, en Arquímedes 2022 (CYPE — adelanta parte de T-14).** Ambos coinciden: raíz 4.074,19 (nuestro 4.074,13 + 0,06 € del redondeo K-por-precio de Presto, dentro del <1 € de D8), DEM010 114,54 con su descomposición respetada, TRA020 8,90 como precio cerrado, acentos/—/€/¦ perfectos.
  - **F7.4b HECHO (2026-06-11):** **D2 códigos deterministas** — la dedupe corre TRAS sanitizar y canonicalizar a cp1252 (los bytes que viajan son la identidad): homónimos divergentes → sufijo `.2`/`.3`; idénticos → concepto compartido (cantidad en la ~D del padre, medición en su ~M); `'——'`/vacíos → generados `P001…` compartiendo generado si son idénticos; una partida no pisa el código de un capítulo/raíz; `%` inicial reservado a conceptos porcentaje. **D3 subcapítulos ANIDADOS reales** vía `core/grouping.groupBySub` (DRY con la vista): las directas van ANTES que los subs en la ~D del capítulo (el anclaje por índice de los ~M lo exige), ~M de las partidas de sub anclado AL SUB con posición `cap\sub\n\`, precio del sub = Σ partidas con K; subs VACÍOS no viajan (el reimport los confundiría con una partida fantasma a 0); asimetría aplanado-al-reimportar documentada y testeada (PEM invariante, mismo orden). **Gate por capas completo (D5+D8):** capa 1 units registro a registro · capa 2 round-trip del seed EXACTO (ahora pasando por el anidado del cap. 1) · **capa 3 fixture Presto ANONIMIZADO commiteado** (`src/core/__fixtures__/presto-mini.bc3`: fechas, ~T raíz vacío, línea de sección, % precio=pct + rendimiento=fracción, capítulo sin ~M, raíz con redondeo K-por-precio → delta −0,30 € < 1 €; re-export→re-import idempotente sin tolerancia) · **capa 4 round-trip del .bc3 REAL local-only `describe.skipIf`** (19 caps/167 partidas: PEM exacto, mediciones y precios estables partida a partida) · capa 5 gate manual SUPERADO (Presto + Arquímedes). **UI:** fila de cabecera «Obra completa» en el ExportModal con chip BC3 (`--state-mq`), wrapper `features/exportar/bc3.ts` (síncrono, sin import dinámico: el writer no lleva librería) + `docFileName` ampliado a `'bc3'`; smoke en Chromium real sin errores de consola. 372 tests verdes. **→ "tu obra abre en Presto".**
  - Aplazado a `TODOS.md`: certs a BC3 (**T-12**), writer upstream (**T-13**), validación CYPE/Arquímedes (**T-14**).
- **Aceptación:** el documento imprimible sale sin controles de UI y con números es-ES; XLSX abre en Excel con celdas numéricas correctas; la cert exportada es REPRODUCIBLE tras editar el presupuesto (snapshot F7.0); el .bc3 exportado pasa el **gate por capas** (D5+D8): round-trips propios EXACTOS al céntimo (seed + fixture anonimizado commiteado + reales local-only), y el **gate manual en Presto real** (estructura + acentos + PEM) — el round-trip contra el propio importer NO basta por sí solo (es lenient/lossy: aplana subcapítulos, puede descartar líneas de medición, charset parcial); validación CYPE/Arquímedes diferida a T-14.
...
### Fase 8 — Pulido
- [x] **F8.0 Unidades de medida editables (HECHO 2026-06-11, petición de dogfood):** la `ud` de la PARTIDA es editable inline en la fila de tabla y en la tarjeta compacta (`editPartidaField 'ud'`, ya existía en el store desde F2 pero la UI la pintaba en solo-lectura); la `ud` del RECURSO ya era editable en la tabla de justificación de escritorio y ahora también en las tarjetas móviles (`editRecurso 'ud'`, banco compartido: cambia en todas las partidas que lo usan; el `%CI` queda fijo en `%`). 3 tests de vista + smoke en Chromium real. 375 tests verdes.
- [x] **F8.1 Responsive + drawer animado + reduced-motion (HECHO 2026-06-11).** QA visual en Chromium real a 390×844 y 768×1024 sobre las 4 vistas + drawer + Referencia + ExportModal: las tarjetas/overlays de F2–F7 ya cubrían casi todo (0 overflow-x). Huecos cazados y corregidos: **(1) TopBar en tablet** — el wordmark «Concreta» se solapaba con las pestañas (el flex encogía la marca por debajo de su contenido); en 760–1023 queda solo el logo (en móvil las pestañas van abajo y el nombre vuelve), y el separador cuelga de `hide-sm` (antes quedaba un sep huérfano en 1080–1180); **(2) cierre del Drawer ANIMADO** — antes desmontaba en seco; ahora `closing` dispara la animación de salida y desmonta en `animationend` (timeout de seguridad 320 ms; con `prefers-reduced-motion` o sin `matchMedia` desmonta al instante; reabrir durante el cierre lo cancela); **(3) Resumen móvil** — los importes grandes partían el `€` a otra línea (`white-space: nowrap` en pemVal/totalVal/bigVal; la etiqueta es la que envuelve). 8 tests nuevos (Drawer estados de cierre + TopBar por breakpoint). 383 tests verdes.
- [x] **F8.2 Accesibilidad (HECHO 2026-06-11).** Lo que YA había de F0–F6 (verificado): anillos `:focus-visible` globales, `role="textbox"`+tabIndex en todos los editables con Enter confirma / Esc cancela, focus-trap en modales (T-7), aria-labels completos (auditado en navegador: **0 controles sin nombre accesible**). Lo NUEVO: **(1) navegación con FLECHAS entre celdas editables** (`hooks/useGridNav` colgado de las tablas de partidas, medición y justificación; las celdas en reposo llevan `data-editcell`; izquierda/derecha por fila, arriba/abajo por columna; dentro del input de edición no intercepta — el caret manda); **(2) contraste AA VERIFICADO computando WCAG**: `--text-secondary` 6,8:1 (oscuro) y 7,6:1 (claro) — pasa AA y AAA, tokens intactos; `--text-disabled` ~2,3:1 queda para texto auxiliar/deshabilitado (exento WCAG; las cabeceras de tabla que lo usan son decisión consciente del sistema visual); **(3) targets táctiles ≥44px** en punteros coarse (`.icon-btn`/`.med-del`). 2 tests de navegación (mueve el foco / no interfiere al editar). 385 tests verdes.
- [x] **F8.3 Estados vacíos + microinteracciones + print (HECHO 2026-06-11; cierra F8).** **Estados vacíos de OBRA** ("Estados de UI de M1" §1): primitiva compartida `components/EmptyState` (dot-grid + tarjeta + CTAs primaria/ghost); Presupuesto con 0 capítulos ya no es una vista EN BLANCO → «Empieza tu primera obra» con [Importar .bc3] (primario, salta a Importar) + [Añadir capítulo]; Certificaciones sin partidas → «Nada que certificar todavía» + [Ir al presupuesto] (journey §4, siguiente paso sutil sin wizard). Los vacíos por-capítulo (F2) y de Resumen (F7.1) ya existían. **Microinteracciones:** `fadeUp` estaba definido en tokens desde F0 pero SIN USO — cableado a las raíces de las 4 vistas (entrada sutil al cambiar de pestaña, verificado en navegador; el reduced-motion global de base.css lo ata). **Print:** los docs de impresión son componentes propios (`features/print`, QA al céntimo en F7.1–F7.3) y no heredan nada del pulido de vistas — verificado que ningún control editable se cuela (los Print* no usan Editable*). 2 tests de estados vacíos. 387 tests verdes.
- **Aceptación:** auditoría a11y básica sin errores graves ✓ (0 controles sin nombre accesible, contraste AA verificado); uso completo con teclado en edición inline ✓ (Tab + flechas + Enter/Esc); impresión limpia en los 3 documentos ✓ (QA F7.1–F7.3, sin controles de UI). **FASE 8 COMPLETA.**
- [x] **Fixes de dogfood en obra real (2026-06-11, «Proyecto de refuerzo de cimentación»):** (1) estados vacíos RECORTADOS — la raíz de la vista no era columna flex y el `flex:1` del vacío no estiraba; clase `viewFill` en las ramas vacías de Presupuesto/Certificaciones; (2) **logo roto en GitHub Pages** — `/favicon.svg` absoluto 404 bajo el base `/concreta-mediciones/`; ahora `import.meta.env.BASE_URL` (verificado con build `GITHUB_PAGES=true` + preview); (3) **inputs de celda que deformaban la tabla al editar** — el ancho intrínseco por defecto de un `<input>` (~20ch) reventaba la columna; `size={1}` en MedNum/MedComment/EditableNum (el ancho lo gobierna la columna); (4) **`UdSelect`**: la unidad deja de ser texto libre (fragmentaba "m2"/"M2"/"m^2") → desplegable con las 10 unidades comunes de obra + «Otra…» libre, en fila/tarjeta de partida y justificación escritorio/móvil; popover `position:fixed` anclado al trigger (los contenedores de tabla llevan overflow que recortaría uno absoluto) — lo que destapó que el `fill both` del fadeUp dejaba un transform residual (matriz identidad ≠ `none`) convirtiendo la vista en containing block y rompiendo cualquier `fixed` interior: fuera el fill-mode. 387 tests verdes.
...
| Codex Review | `/codex review` | Independent 2nd opinion | 7 | REVIEWED | run 1: integer-cents/BC3/cert-first. run 2 (CEO): 18 puntos. run 3 (Eng F1): 2 bugs reales. run 4 (Eng F4 plan, 2026-06-10): 14 problemas; 2 tensiones cross-model reales (encoding de líneas frágil → snapshot; contradictorios → cert-local) + refinó addCert/MedLine.id/%-no-céntimos. run 5 (Eng F6): 16 hallazgos, 1 bug de corrupción (ids). run 6 (Eng F7): tensiones snapshot-precio/Resumen resueltas. run 7 (Eng F7.4, 2026-06-11): 15 hallazgos, 1 error factual cazado (~K 7 grupos), 12 refinamientos de spec absorbidos, 2 tensiones resueltas (gate al céntimo dual; bala trazadora) |
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 7 | CLEAR (F7.4 plan) | run 1 (plan): 6 issues, scope→M1. run 2 (post-F0): 3. run 3 (post-F1): 8 decisiones, 124 tests. run 4 (F4 plan): 6 decisiones, 1 crítico cazado. run 5 (F6 plan): idb-keyval/UUID/hidratación gated. run 6 (F7 plan): troceo F7.0–F7.4, snapshot de precio. run 7 (F7.4 plan, 2026-06-11): 5 issues (spec fijada con ground truth, dedupe de códigos, subcaps anidados, DRY medición, gate por capas), 0 sin resolver |
...
- **POST-F7.0–F7.3 / F7.4-PLAN (eng run 7, 2026-06-11 — revisión de PLAN de F7.4):** F7.0–F7.3 shipeadas (326 tests; snapshot de precio, PDF, XLSX, DOCX). F7.4 = serializador FIEBDC-3 propio (no existe en JS — la lib `bc3` es parser-only, verificado). **Step 0:** scope aceptado tal cual (1 módulo core nuevo, ~6 ficheros, smell no dispara); ground truth local (fixture Presto 8.7 + parser) usado para CERRAR la spec en la review en vez de aplazarla. **5 decisiones (ver §Fase 7 F7.4):** D1 spec núcleo fijada (cp1252/ANSI, punto decimal, CRLF, ~K, raíz=PEM con K, sanitización); D2 renombrado determinista de códigos (homónimos divergentes/`'——'`); D3 subcapítulos ANIDADOS (asimetría con el import documentada); D4 DRY `lineParcial` (+drive-by en bc3import); D5 gate de fidelidad por capas (fixture anonimizado commiteado + reales local-only + manual Presto — los reales NO están en git, datos de cliente). **Voz externa Codex (run 7):** 15 hallazgos, cero contradicción con D1–D5; cazó 1 error factual (~K de 7 grupos, no 8), 12 refinamientos de spec absorbidos (D7: anclaje ~M, %CI, tipos ~C, precios de capítulo, dedupe 2ª pasada, huérfanos, fileName, regresión dims-0) y 2 tensiones resueltas por el fundador: D8 gate al céntimo DUAL (exacto en round-trips propios; <1 € solo vs raíz de Presto) y D9 **bala trazadora** (writer mínimo → Presto real ANTES de construir el resto). 3 TODOs nuevos (**T-12** certs a BC3, **T-13** writer upstream, **T-14** validación CYPE). **0 sin resolver.**
- **VERDICT (F7.4):** **F7.4 PLAN CLEARED** (Eng run 7 + Codex run 7). F1–F6 y F7.0–F7.3 cerradas. **Próximo paso: implementar F7.4a** (writer mínimo según spec D1 + trazadora en Presto real); F7.4b tras el GO. Trabajo diferido en `TODOS.md` (T-12/T-13/T-14).
