...
- [ ] **F7.4 BC3 export FIEBDC-3 — PLAN CERRADO (eng-review 2026-06-11, 5 issues D1–D5 + voz externa Codex: 15 hallazgos → 12 refinamientos absorbidos D7 y 2 tensiones resueltas D8/D9; 0 sin resolver). Troceado en BALA TRAZADORA (D9, Codex):** **F7.4a** = spec + writer mínimo (raíz+1 cap+2 partidas con ~D/~M/~T) → **abrirlo en Presto REAL** → GO/ajustar · **F7.4b** = dedupe + anidado + huérfanos + UI + gates por capas.
  ...
  - **F7.4a HECHO (2026-06-11):** writer `src/core/bc3export.ts` (puro: `obraToBc3(Bc3ExportObra) → Uint8Array`; encoder cp1252 propio con extras 0x80–0x9F, sanitización `|¦`/`\→/`/`~` neutralizado en ~T, ~V/~K/~C/~D/~M/~T según D1+D7; raíz = PEM CON K y capítulo = Σ hijos CON K — verificado contra el fixture: en el propio Presto Σ capítulos = precio raíz al céntimo con las partidas en base; UN ~M por partida en orden para el anclaje por índice, con uds=1 explícito cuando las 4 dims van vacías para que el import no la descarte como línea de sección). **29 tests:** spec registro a registro (encoder, ~K 7 grupos ±pct, sanitización, tipos ~C, %CI=porcentaje, huérfanos fuera) + **round-trip del seed EXACTO al céntimo** (PEM 26.291,91, delta raíz 0, precios/cantidades/rendimientos/mediciones idénticos, con y sin K) + regresión dims-0 (D4). **Bala trazadora `docs/trazadora-presto.bc3`** commiteada (sintética, sin datos de cliente): el test la regenera si el writer cambia (el diff de git delata cualquier cambio de formato). Nota de entorno: el `TextDecoder('windows-1252')` de Node small-icu decodifica 0x80–0x9F como C1 → esos chars se validan a nivel de byte (en navegador real es cp1252 correcto). 355 tests verdes. **PENDIENTE (gate manual D5 capa 5): abrir la trazadora en Presto real — estructura, acentos, PEM 604,93 € — y dar el GO a F7.4b.**
