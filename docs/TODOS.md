...
- ~~Pendiente menor: reordenar contenedores por drag&drop (el menú "Mover a" cuelga al FINAL del destino; no hay orden fino entre hermanos).~~ **HECHO (2026-08-05, feedback de obra)**: `reorderContainer(nodeId, beforeId)` / `moveContainerBy(nodeId, ±1)` reordenan capítulos y subcapítulos ENTRE HERMANOS (reparentar sigue siendo `moveSubtree`), con renumeración en cascada (capítulos a 1..N rebasando la ruta de sus subs; subs recodificados bajo su padre; `pos` de las partidas afectadas). UI: arrastrar la fila del árbol + «Subir/Bajar» en los menús ⋮ (el del capítulo es nuevo: `ChapterMenu`, que además recoge «Eliminar capítulo», antes un icono a un clic en la tarjeta). Gemelo para partidas: `reorderPartida`/`movePartidaBy` + asa de arrastre en la columna Nº.
