/* ===========================================================================
   ai/chatHistory — construcción de los turnos de request desde el hilo de la UI.
   ---------------------------------------------------------------------------
   Portado verbatim de concreta-v2. La UI conserva TODOS los ítems; aquí solo se
   recorta lo que viaja en cada petición: ventana deslizante de turnos (podada por
   pares para garantizar la alternancia estricta user/assistant) y cupo total de
   imágenes (podadas del turno más antiguo hacia delante). Función PURA.

   El manejo de imágenes se conserva íntegro aunque la visión llegue en F-A5:
   ChatTurn ya las contempla y evita un re-port. Sin imágenes, las ramas de cupo
   son no-ops.
   =========================================================================== */
...
/**
 * Forma mínima ESTRUCTURAL de un ítem del hilo de la UI. Mantiene esta función
...
export interface ChatItemLike {
  ...
  reply?: string;
  /** JSON crudo del envelope de la respuesta anterior (se reenvía verbatim). */
  rawEnvelope?: string;
}
 ...
 *   user superviviente (marcador IMAGES_CARRIED_MARKER); solo el cupo las poda.
 * - Cupo MAX_REQUEST_IMAGES por request: se podan del turno más antiguo hacia
 *   delante; un turno podado añade IMAGE_OMITTED_MARKER.
 */
