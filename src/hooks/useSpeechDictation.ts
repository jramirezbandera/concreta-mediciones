/* ===========================================================================
   hooks/useSpeechDictation — dictado por voz nativo (Web Speech API), es-ES.
   ---------------------------------------------------------------------------
   Portado de concreta-v2 (F-A6). Sin coste ni clave: la transcripción la hace el
   propio navegador (Chrome/Edge/Safari; Firefox NO la implementa → `supported`
   false y el consumidor oculta el micro, degradación limpia). Idioma fijo es-ES.

   El hook NO toca ningún estado de texto: en cada evento emite el transcrito
   ACUMULADO desde que empezó la escucha, partido en `final` (segmentos cerrados)
   e `interim` (lo que aún se reconoce). El consumidor decide cómo insertarlo (aquí:
   base + final + interim), de modo que el interino se ve en vivo y se reescribe en
   cada evento sin duplicar.

   Modo continuo (toggle): Chrome corta la sesión por inactividad y dispara `onend`;
   mientras no sea una parada manual, se reanuda para que el dictado no se detenga
   solo. El acumulado `final` vive en un ref y persiste entre reinicios; el índice
   de resultados del evento se reinicia, por eso se recorre desde `resultIndex`
   acumulando solo los `isFinal` nuevos.
   =========================================================================== */
...
export function useSpeechDictation(opts: SpeechDictationOptions): SpeechDictation {
  ...
  // Cortar el reconocedor al desmontar (cierre del panel, cambio de vista…).
  useEffect(() => {
    return () => {
      manualStopRef.current = true;
      listeningRef.current = false;
